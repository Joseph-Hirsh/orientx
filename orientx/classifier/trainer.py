...
from ..printer.classifier_printer import (
...
class ClassificationPipeline:
    def __init__(self, model_name, num_classes=2, max_length=128, batch_size=16, lr=2e-5, epochs=5):
    ...
    def train(self, train_output_path):
        print_classifier_training_heading(self.epochs)
        optimizer = torch.optim.AdamW(self.model.parameters(), lr=self.lr)
        total_steps = len(self.train_loader) * self.epochs
        scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=0, num_training_steps=total_steps)

        for epoch in range(self.epochs):
            print_classifier_epoch_update(epoch, self.epochs)
            self.model.train()
            ...
            print_classifier_loss_metric(epoch, loss)

        self.save_model(train_output_path)
        accuracy, report = self.evaluate()

        print_classifier_training_complete(accuracy, report, train_output_path)
